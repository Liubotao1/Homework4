...
public class CreateDirAndFileAppMain {
    ...
    // 不同操作系统的同学可以更改这个值，比如mac或者linux可以写为~代表home目录
    private static final String ROOT = "." + File.separator;
    private static Scanner scanner = new Scanner(System.in);

    public static void main(String[] args) throws IOException {
        // TODO 使用File类，依次创建多层文件夹，修改文件夹名字，在指定文件夹创建文件，删除文件，删除文件夹
        File dir = createDirs();   //创建多级文件夹

        File newDir = renameDir(dir);   //修改文件夹名称
        if (newDir == null) return;

        String fileName = createFiles(newDir);   //创建文件

        String fileNameNew = renameFiles(newDir, fileName);  //修改文件名称

        deleteFiles(newDir, fileNameNew);  //删除文件

        deleteDir(newDir);///\删除文件夹
    }
